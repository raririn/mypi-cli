...
import { redactCredentialPayload, withCredentialRedaction } from "./credential-redaction.ts";
...
export function stream<TApi extends Api>(
	...
	const safeContext = redactCredentialPayload(context).value;
	const safeOptions = withCredentialRedaction(withEnvApiKey(model, options));
	...
	if (builtinProvider) {
		if (model.provider.startsWith("cloudflare-") && !hasResolvedCloudflareAuth(options)) {
			return compatModels.stream(model, safeContext, safeOptions as ModelsApiStreamOptions<TApi>);
		}
		return builtinProvider.stream(model, safeContext, safeOptions as ApiStreamOptions<TApi>);
	}
	const provider = resolveApiProvider(model.api);
	return provider.stream(model, safeContext, safeOptions);
}
...
export function streamSimple<TApi extends Api>(
	...
	if (builtinProvider) {
		if (model.provider.startsWith("cloudflare-") && !hasResolvedCloudflareAuth(options)) {
			return compatModels.streamSimple(model, safeContext, safeOptions);
		}
		return builtinProvider.streamSimple(model, safeContext, safeOptions);
	}
	const provider = resolveApiProvider(model.api);
	return provider.streamSimple(model, safeContext, safeOptions);
}
