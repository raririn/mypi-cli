...
export class SkillInvocationMessageComponent extends Box {
	...
	private updateDisplay(): void {
		...
		if (this.expanded) {
			// Expanded: loaded-skill label + skill name header + full content
			const label = theme.fg("customMessageLabel", `\x1b[1m[skill loaded]\x1b[22m`);
			...
			// Collapsed: single line - [skill loaded] name (hint to expand)
			const line =
				theme.fg("customMessageLabel", `\x1b[1m[skill loaded]\x1b[22m `) +
