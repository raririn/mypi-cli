...
describe("AgentSession.getSessionStats", () => {
	...
	it("treats a post-compaction assistant message with missing usage as unknown", async () => {
		const { session, sessionManager } = await createSession();

		try {
			const keptUserId = sessionManager.appendMessage(createUserMessage("before compaction", 1));
			sessionManager.appendMessage(createAssistantMessage("response", 1_000, 2));
			sessionManager.appendCompaction("summary", keptUserId, 1_000);
			sessionManager.appendMessage(createUserMessage("after compaction", 3));
			const assistantWithoutUsage = createAssistantMessage("legacy response", 100, 4) as Partial<AssistantMessage>;
			delete assistantWithoutUsage.usage;
			sessionManager.appendMessage(assistantWithoutUsage as AssistantMessage);
			syncAgentMessages(session, sessionManager);
