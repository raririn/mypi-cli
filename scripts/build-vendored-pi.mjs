...
const provenance = JSON.parse(readFileSync(join(root, "vendor", "pi", "MYPI_PROVENANCE.json"), "utf8"));
const packages = [
...
if (typeof provenance.upstreamVersion !== "string") {
  throw new Error("Vendored Pi provenance is missing upstreamVersion.");
}
...
for (const [packageName] of packages) {
  ...
  const manifest = JSON.parse(
    readFileSync(join(root, "vendor", "pi", "packages", packageDirectory, "package.json"), "utf8"),
  );
  if (manifest.name !== packageName || manifest.version !== provenance.upstreamVersion) {
    throw new Error(
      `Vendored ${packageName} must match provenance version ${provenance.upstreamVersion}; found ${manifest.name}@${manifest.version}.`,
    );
  }
}
...
console.log(`Built vendored Pi ${provenance.upstreamVersion} from source.`);
