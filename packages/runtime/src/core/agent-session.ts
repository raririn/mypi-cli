...
interface MyPiSkillInvocationPresentation {
	version: 1;
	skillName: string;
	originalText: string;
}
...
function summaryHasStructuredUnfinishedWork(summary: string): boolean {
	const section = summary.match(/(?:^|\n)### In Progress\s*\n([\s\S]*?)(?=\n### |\n## |$)/i)?.[1] ?? "";
	return /^\s*[-*+]\s+\[\s\]\s+\S/m.test(section);
}
...
export function mypiShouldContinueAfterThresholdCompaction(input: MyPiCompactionContinuationInput): boolean {
	...
	return summaryHasStructuredUnfinishedWork(input.summary);
}

export function createMyPiProactiveContinuation(compactionId: string) {
	return {
		role: "custom" as const,
		customType: "mypi-proactive-compaction-continuation",
		content: MYPI_PROACTIVE_CONTINUATION_TEXT,
		display: false,
		details: { version: 1, compactionId },
	};
}
...
export interface PromptOptions {
	...
	/** MyPi host guard: reject instead of starting a normal prompt if streaming ended during preflight. */
...
export class AgentSession {
	...
	private async _mypiEmitDispatchFailureIfUnsettled(settlementEpoch: number, error: unknown): Promise<void> {
		if (this._mypiSettlementEpoch !== settlementEpoch) {
			return;
		}
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// command expansion. A trusted input hook may still expand the original
			// explicit /skill command, so retain presentation provenance from that input.
			const skillPresentation =
				this._skillInvocationPresentation(currentText) ??
				(options?.source === "extension" ? this._skillInvocationPresentation(text) : undefined);
			...
			// MyPi queue mutations must never fall through into a new normal prompt
			// if the active run settles while async input hooks are executing.
			if (options?.requireStreaming && !this.isStreaming) {
				throw new Error(
					"Session finished streaming before the queued message could be accepted. Retry to send it as a new turn.",
				);
			}
			...
			if (this.isStreaming) {
				...
				if (options.streamingBehavior === "followUp") {
					await this._queueFollowUp(expandedText, currentImages, skillPresentation, options?.mypiQueuedMessageId);
				} else {
					await this._queueSteer(expandedText, currentImages, skillPresentation, options?.mypiQueuedMessageId);
				}
				preflightResult?.(true);
				return;
			}
	...
	private _skillInvocationPresentation(text: string): MyPiSkillInvocationPresentation | undefined {
		...
		return skill ? { version: 1, skillName, originalText: args || text } : undefined;
	}
	...
	async steerWithId(text: string, images?: ImageContent[]): Promise<string> {
		...
		const skillPresentation = this._skillInvocationPresentation(text);
	...
	async setModel(model: Model<any>, options?: { persistGlobal?: boolean }): Promise<void> {
		...
		if (options?.persistGlobal === true) {
			const { updateDefaultModel } = await import("../product/global-config.ts");
			await updateDefaultModel(`${model.provider}/${model.id}`, join(this._agentDir, "config.yaml"));
		}
	...
	private async _runAutoCompaction(reason: "overflow" | "threshold", willRetry: boolean): Promise<boolean> {
		...
		try {
			...
			const compactionId = savedCompactionEntry?.id;
			const shouldContinue = mypiShouldContinueAfterThresholdCompaction({
				reason,
				willRetry,
				aborted: false,
				...
				compactionId,
				summary,
				branchEntries: newEntries,
			});
			if (shouldContinue && compactionId !== undefined && !this._mypiProactiveContinuationTokens.has(compactionId)) {
				this._mypiProactiveContinuationBudget--;
				this._mypiProactiveContinuationTokens.add(compactionId);
				this.agent.followUp({
					...createMyPiProactiveContinuation(compactionId),
					timestamp: Date.now(),
				});
				return true;
			}
			...
			// Existing queues win over proactive continuation and retain their original order.
			return this.agent.hasQueuedMessages();
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : "compaction failed";
			this._mypiSettledOutcome = { kind: "compaction-error", errorMessage };
			if (started) {
				this._emit({
					type: "compaction_end",
					reason,
	...
	private getExtensionSourceLabel(extensionPath: string): string {
		if (extensionPath.startsWith("<builtin:")) {
			return "builtin";
		}
		...
		runner.bindCore(
			{
				sendMessage: (message, options) => {
					const settlementEpoch = this._mypiSettlementEpoch;
					this.sendCustomMessage(message, options).catch(async (err) => {
						runner.emitError({
