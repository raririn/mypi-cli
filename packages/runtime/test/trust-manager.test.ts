...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".mypi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".mypi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".mypi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".mypi"), { recursive: true });
			writeFileSync(join(cwd, ".mypi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".mypi"), { recursive: true, force: true });
