...
import { archiveChat, deleteArchivedChat, eraseChatAssets, listChats, restoreChat, type ChatStorageRecord } from "./mypi-chat-storage.ts";
...
export default function chatManageExtension(pi: ExtensionAPI): void {
  // Grant retired (beta.13): the tools are governed by Settings → Tools
  // ("chat-manage" group, default off); the central registry filter removes
  // them entirely when the group is off.
  const toolsPresent = () => pi.getActiveTools().some((name) => TOOL_SET.has(name));
  ...
  async function handle(args: string, ctx: ExtensionContext): Promise<void> {
    ...
    if (!toolsPresent()) {
      ctx.ui.notify('The chat-management tools are disabled. Enable "Chat management" in Settings → Tools (new sessions pick it up).', "warning");
      return;
    }
  ...
  pi.on("before_agent_start", (event) => toolsPresent() ? {
