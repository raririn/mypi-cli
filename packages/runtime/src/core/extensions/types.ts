...
export interface ExtensionUIDialogOptions {
	...
	timeout?: number;
	/** MyPi host metadata for a trusted structured AskUser interaction. */
	mypiAskUser?: unknown;
}

export type StartupResourceSection = "context" | "skills" | "prompts" | "extensions" | "themes";
...
export interface ExtensionUIContext {
	...
	/** Filter the built-in startup resource listings (interactive mode only). */
	setStartupResourceSections?(sections: readonly StartupResourceSection[] | undefined): void;

	/** Open an authoritative scrollable resource viewer (interactive mode only). */
	showResourceSections?(sections: readonly StartupResourceSection[], title?: string): Promise<void>;
...
export interface ExtensionContext {
	...
	/** Navigate the active session tree. Hosts must expose this only through an explicit user action. */
	navigateTree(
		targetId: string,
...
export type AgentSettledOutcome =
	| { kind: "success" }
	| { kind: "aborted" }
	| { kind: "error" | "compaction-error"; errorMessage?: string };
...
export type InlineExtension =
	...
	| {
			...
			hidden?: boolean;
			/** Runtime-owned extension that cannot be disabled through package settings. */
			builtIn?: boolean;
	  };
