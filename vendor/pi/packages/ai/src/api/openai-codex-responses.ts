...
export const stream: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	(async () => {
		...
		try {
			...
			// Copy into an ArrayBuffer-backed view accepted by both DOM and Node fetch BodyInit types.
			const sseBody: ArrayBuffer | string = compressedBody ? Uint8Array.from(compressedBody).buffer : bodyJson;
