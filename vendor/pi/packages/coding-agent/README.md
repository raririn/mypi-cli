...
| Shift+Tab | Cycle safety mode by default (`/shift-tab thinking` switches it for the current process) |
...
Replace the default system prompt with `.mypi/SYSTEM.md` (trusted project) or `~/.mypi/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`. For a deliberately shorter replacement, copy `docs/system-prompts/minimal.md` to one of those `SYSTEM.md` locations; there is no prompt-preset switch.
...
| `--output-schema <file>` | Return a validated JSON Schema result in print/JSON mode (see [docs/structured-output.md](docs/structured-output.md)) |
...
Available built-in tools: `read`, `bash`, `edit`, `write`, `grep`, `find`, `ls`, `commentary`
