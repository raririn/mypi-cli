...
import type { ControllerCallbacks } from '../../../src/product/gui-control/controller.ts'
import { MYPI_CONTROL_PROTOCOL, type ClientFrame, type ServerFrame } from '../../../src/product/gui-control/protocol.ts'
import type { SocketTransport, SocketTransportCallbacks } from '../../../src/product/gui-control/transport.ts'

const controllerBuild = await build({
  entryPoints: [join(import.meta.dirname, '../../../src/product/gui-control/controller.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  write: false,
...
const { GuiControlController } = await import(controllerUrl) as typeof import('../../../src/product/gui-control/controller.ts')
